...
const UserDashboard = () => {
  ...
  const getPaymentBadge = (paymentStatus) => {
    ...
    return (
      <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium" style={{ backgroundColor: isPaid ? '#dcfce7' : '#fef3c7', color: isPaid ? '#166534' : '#92400e' }}>
  ...
  const filterEnrollments = () => {
    ...
    return enrollments.filter(e => {
      ...
      const isPast = workshopDate <= now || e.status === 'completed';

      if (activeTab === 'upcoming') return !isPast && e.status !== 'cancelled' && e.status !== 'rejected';
            ...
            <div className="flex space-x-8 px-6">
              {['upcoming', 'past', 'all'].map((tab) => (
                <button
                  ...
                  className={`py-4 px-1 border-b-2 font-medium text-sm transition-colors ${activeTab === tab ? 'border-orange-500 text-orange-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'}`}
                >
            ...
            {filteredEnrollments.length === 0 ? (
              ...
              <div className="space-y-4">
                {filteredEnrollments.map((enrollment) => (
                  <div key={enrollment._id || enrollment.id} className="border border-gray-200 rounded-lg p-6 hover:shadow-md transition-shadow">
                    <div className="flex flex-col md:flex-row md:items-center md:justify-between">
                      <div className="flex-1 mb-4 md:mb-0">
                        <div className="flex items-start justify-between mb-2">
                          <h3 className="text-lg font-semibold text-gray-900">
                            {enrollment.workshopId?.title || enrollment.workshopTitle}
                          </h3>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-4">
                          <div className="flex items-center text-sm text-gray-600">
                            ...
                            <span>{formatDate(enrollment.workshopId?.date || enrollment.workshopDate)}</span>
                          </div>
                          <div className="flex items-center text-sm text-gray-600">
                            <Users className="h-4 w-4 mr-2 text-gray-400" />
                            <span>{enrollment.slots || enrollment.participants} {t('dashboard.participants')}</span>
                          </div>
                          <div className="flex items-center text-sm text-gray-600">
                          ...
                          <div className="flex items-center text-sm">
                            ...
                            {getPaymentBadge(enrollment.paymentStatus || 'pending')}
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="flex flex-col items-end space-y-2">
                        ...
                        <div className="flex gap-2">
                          {/* Only show E-Ticket if the shop owner has CONFIRMED the registration */}
